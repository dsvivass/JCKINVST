...
def OrganizadorDfDia(archivo):
    ...
    df_cierre = df_dia.pivot(index='Fecha', columns='Hora', values='P_cierre')
    return df_pivot, df_cierre
...
def Graficar(DatosPredict, DatosReales, DatosPredict_cierre, DatosReales_cierre):


    ax.plot(horas, DatosPredict, 'r-', label='Datos Inteligencia Artificial')
    ax.plot(horas, DatosReales, 'b-', label='Datos Reales')
    ax.plot(horas, DatosPredict_cierre, 'g--', label='Datos Inteligencia Artificial Cierre')
    ax.plot(horas, DatosReales_cierre, 'k--', label='Datos Reales Cierre')
    ...
    ax.legend(custom_lines, ['Datos Reales', 'Datos Inteligencia Artificial', 'Datos Inteligencia Artificial Cierre', 'Datos Reales Cierre'])
...
# LINEA PRINCIPAL
df, df_cierre = pd.DataFrame(), pd.DataFrame() # Creacion dataframe que almacena todos los datos
DirAct = os.getcwd()
os.chdir('DatosHistoricos/AAPL')
ls = sorted(os.listdir())

for file in ls:
    df_org, df_org_cierre = OrganizadorDfDia(file)
    df = df.append(df_org)
    df_cierre = df_cierre.append(df_org_cierre)

df = OrganizadorDfGeneral(df)
df_cierre = OrganizadorDfGeneral(df_cierre)

x = df[CadenaHoras(HInicial=(4,0,0), HFinal=(8,20,0), paso_minutos=5)]
x_cierre = df_cierre[CadenaHoras(HInicial=(4,0,0), HFinal=(8,20,0), paso_minutos=5)]
...
fig, ax = plt.subplots()
i, DatosReales, DatosPredict, DatosReales_cierre, DatosPredict_cierre = 1, [], [], [], []
horas = CadenaHoras(HInicial=(8,30,0), HFinal=(15,0,0))
for hora in horas:
    tini2 = time.time()
    y = df[[hora]]
    y_cierre = df_cierre[[hora]]
    ...
    input_cierre = [('polinomio', PolynomialFeatures(degree=2)), ('regresion', Ridge(alpha=1))]
    pipe_cierre = Pipeline(steps=input_cierre)
    pipe_cierre.fit(x_cierre, y_cierre)
    Predict_cierre = pipe_cierre.predict(df_org_cierre[x_cierre.columns])
    DatosPredict_cierre.append(Predict_cierre[0][0])
    # try:
    #     DatosPredict.append(Predict[0][0])
    # except:
    ...
    try:
        ...
        DatosReales_cierre.append(df_org_cierre.loc[df_org_cierre.index[0], hora])
    except:
        DatosReales.append(df_org.loc[df_org.index[0]].max())
        DatosReales_cierre.append(df_org_cierre.loc[df_org_cierre.index[0]].max())
    print(f'[MEJOR AJUSTE: {i} de {len(horas)}], Tiempo iteracion {time.time() - tini2}: ', )
    i=i+1

print(f'[TIEMPO TOTAL: {time.time() - tini}]')
Graficar(DatosPredict, DatosReales, DatosPredict_cierre, DatosReales_cierre)