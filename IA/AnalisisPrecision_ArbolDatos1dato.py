...
def OrganizadorDfDia(archivo, horas=False):
    ...
    # df_dia.loc[df_dia['P_cierre'] >= df_dia['P_apert'], 'CondicionReal'] = 'Sube'
    # df_dia.loc[df_dia['P_cierre'] < df_dia['P_apert'], 'CondicionReal'] = 'Baja'
    ...
    df_dia.set_index(df_dia['Hora'], inplace=True)
    # print(df_dia)
    # print(df_dia.loc['03:30:00', 'P_alto'])
    # print(df_dia.loc['08:30:00', 'P_cierre'] >= df_dia.loc['08:30:00', 'P_apert'])
    # if horas is False:
    #     if df_dia.loc['08:30:00', 'P_cierre'] >= df_dia.loc['08:30:00', 'P_apert']:
    #         df_dia['Condicion'] = 'Sube'
    #     else:
    #         df_dia['Condicion'] = 'Baja'
        # df_dia.loc[df_dia.loc['08:30:00', 'P_cierre'] >= df_dia.loc['08:30:00', 'P_apert'], 'Condicion'] = 'Sube'
        # df_dia.loc[df_dia.loc['08:30:00', 'P_cierre'] < df_dia.loc['08:30:00', 'P_apert'], 'Condicion'] = 'Baja'

    # print(df_dia[['P_cierre', 'P_apert', 'Condicion']])

    df_dia['Ponderado'] = (df_dia['P_alto'] + df_dia['P_bajo'] + df_dia['P_apert'] + df_dia[
        'P_cierre']) / 4  # ECUACION CAMBIANTE
    if horas is not False:
        df_dia = df_dia[df_dia['Hora'].isin(horas)]
    ...
    if horas is False:
        if df_dia.loc['08:30:00', 'P_cierre'] >= df_dia.loc['08:30:00', 'P_apert']:
            df_pivot['Condicion'] = 'Sube'
        else:
            df_pivot['Condicion'] = 'Baja'
...
def MejorAjuste(profundidad, cv, x, y):
    ...
    input = [('escala', StandardScaler()), ('arbol', DecisionTreeClassifier(criterion='entropy'))]
    parametros = [{'arbol__max_depth':profundidad}]
    pipe = Pipeline(steps=input)
    grid = GridSearchCV(pipe, parametros, cv=cv, n_jobs=-1, return_train_score=True, scoring='accuracy', verbose=2)
    grid.fit(x,y)
    scores = grid.cv_results_
    print(scores)
    r2 = None
    for n_prof, mean_train, mean_test in zip(scores['param_arbol__max_depth'],
                                                scores['mean_train_score'], scores[
                                                    'mean_test_score']):  # , norm, scores['param_regresion__normalize'],
        if r2 is None or mean_test >= r2:
            r2 = mean_test
            n_profundidadR = n_prof
            mean_trainR = mean_train
            mean_testR = mean_test
    # print(grid.best_estimator_)
    print(f'[ESTADO MEJOR AJUSTE: Numero de profundidad: {n_profundidadR}, media r2 entrenamiento: {mean_trainR}, '
          f'media r2 pruebas: {mean_testR}]')  # , normalizado, {norm}
    return n_profundidadR
...
for i,archivo_test in enumerate(ls[-256:]): # enumerate(ls[-256:])
    ...
    for vecino in [100]:
        ...
        for hora in horas:
            ...
            # MejorArbol = MejorAjuste(profundidad=np.arange(10,100,10), cv=4, x=x, y=y)

            input = [('escala', StandardScaler()), ('arbol', DecisionTreeClassifier(criterion='entropy', max_depth=2000))]
            pipe = Pipeline(steps=input)
            pipe.fit(x, y)
        ...
        print(df_actual['Condicion'].values)
