...
def MejorAjuste(kernel, cv, x, y):
    ...
    input = [('svmm', SVC())]
    parametros = [{'svmm__kernel':kernel}]
    ...
    print(scores)
    r2 = None
    for Kernel, mean_train, mean_test in zip(scores['param_svmm__kernel'],
    ...
    print(f'[ESTADO MEJOR AJUSTE: Kernel: {KernelR}, media r2 entrenamiento: {mean_trainR}, '
...
for i,archivo_test in enumerate(ls[-256:]): # enumerate(ls[-256:])
    ...
    for vecino in [100]:
        ...
        for hora in horas:
            ...
            MejorArbol = MejorAjuste(kernel=['linear', 'poly', 'rbf', 'sigmoid'], cv=4, x=x, y=y)
