...
def MejorAjuste(profundidad, cv, x, y):
    ...
    grid = GridSearchCV(pipe, parametros, cv=cv, n_jobs=-1, return_train_score=True, scoring='accuracy', verbose=1)
    grid.fit(x,y)
...
for i,archivo_test in enumerate(ls[-256:]): # enumerate(ls[-256:])
    ...
    for vecino in [100]:
        ...
        df_pred, df_pred_prob = pd.DataFrame(), pd.DataFrame()
        for hora in horas:
            tini2 = time.time()
            y = df[['Condicion']].values.ravel()
            # print(f'[ENCONTRANDO MEJOR GRADO Y ALFA: {i} de {len(horas)}')
            # MejorArbol = MejorAjuste(profundidad=np.arange(1,100,10), cv=4, x=x, y=y)

            input = [('escala', StandardScaler()), ('regresion', LogisticRegression(C=0.01, solver='liblinear'))]
            pipe = Pipeline(steps=input)
            pipe.fit(x, y)
            Predict = pipe.predict(df_org[x.columns])
            Predict_prob = pipe.predict_proba(df_org[x.columns])
            # print('x', x)
            # print('y', y)
            ...
            df_pred_prob = df_pred_prob.append(pd.DataFrame(Predict_prob), ignore_index=True)
            i=i+1
